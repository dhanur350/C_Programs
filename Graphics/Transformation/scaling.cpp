...
{
    int temp[2][1] = {0}; // NULL set

    for (int i = 0; i < 2; i++)         // iteration till index 2
        for (int j = 0; j < 1; j++)     // iteration till index 1
            for (int k = 0; k < 2; k++) // iteration till index 2
                temp[i][j] += (s[i][k] * p[k][j]);
    // Here (s[i][k] * p[k][j]) will scale it to final coordinate and then add it to changed coordinate temp[i][j]
